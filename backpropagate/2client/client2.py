...
# Use only half of the data for client 1
x_train = x_train[75:]
y_train = y_train[75:]
...
for epoch in range(15):  # Adjust epochs as needed
    ...
    response = requests.post("http://localhost:5001/process_activations", json={"client_id": 1, "activations": initial_activations.numpy().tolist()})
    ...
    if grads_initial_activations is None:
        ...
        response = requests.post("http://localhost:5001/backpropagate", json={"client_id": 1, "grads": grads_initial_activations.numpy().tolist()})
...
try:
    ...
    response = requests.post("http://localhost:5001/process_activations", json={"client_id": 1, "activations": x_test_client.tolist()})
