...
client2_x_train = x_train[:split_index]
client2_y_train = y_train[:split_index]
print("Client 2 x_train shape : ", client2_x_train.shape)
...
for epoch in range(1):
    ...
    initial_activations = initial_model(client2_x_train, training=False)
    ...
    response = requests.post("http://localhost:5002/process_activations", json={"client_id": 2, "activations": initial_activations.numpy().tolist()})
    ...
    final_model.fit(server_activations, client2_y_train[:len(server_activations)], epochs=1, verbose=1)
    logging.info('Final model training step complete.')
    
    with tf.GradientTape() as tape:
        predictions = final_model(server_activations, training=True)
        loss = tf.keras.losses.sparse_categorical_crossentropy(client2_y_train[:len(server_activations)], predictions)
    ...
    with tf.GradientTape() as tape:
        ...
        loss = tf.keras.losses.sparse_categorical_crossentropy(client2_y_train[:len(initial_activations)], predictions)
    ...
    if grads_initial_activations is None:
        ...
        response = requests.post("http://localhost:5002/backpropagate", json={"client_id": 2, "grads": grads_initial_activations.numpy().tolist()})
        ...
        with tf.GradientTape() as tape:
            initial_outputs = initial_model(client2_x_train, training=True)
    ...
    train_loss, train_accuracy = final_model.evaluate(server_activations, client2_y_train[:len(server_activations)], verbose=0)
...
try:
    ...
    response = requests.post("http://localhost:5002/process_activations", json={"client_id": 2, "activations": x_test_client.tolist()})
